...
- **Релеи собеседников дозваниваются заранее:** при старте для недавних
  ...
  круг (~3 с). RTT e2e «релеи внутри ботов»: 64 → 35 → 12,8 с.

Не проверено ничем, кроме CI: ни одного слушающего сокета на устройстве
владельца (`ss -ltnp`, `lsof -i`, `/proc/net/tcp` на Android); Android и macOS
...
1. **Сид сети релеев — ждёт владельца.** Фаза 5 влита (#98) и вышла в 0.4.11;
   ...
   **Нужно от владельца:** приватный ключ `age` целиком в секрет
   `DHT_SEED_AGE_KEY` — в *Settings → Secrets and variables → Actions*, вкладка
   **Secrets**, не Variables (токен `gh` здесь не может ни читать, ни писать ни
   секреты, ни переменные); после первого запуска relay-testnet с ним — destination
   сида в переменную `GIPNY_DHT_SEEDS`; затем релиз. До этого в сборках сида нет,
