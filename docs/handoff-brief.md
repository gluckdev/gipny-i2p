...
Проверено локально: `cargo check` для `gipny-libcore`, `gipny-agent`,
`gipny-bot`, `e2e-harness` и `gipny`; 60 тестов `gipny-libcore`, 16 unit + 3
simulation-теста `gipny-dht`.
...
1. **Фаза 5 сети релеев — код в ветке `feat/dht-seed`, ждёт CI и владельца.**
   ...
   `release.yml` вшивает `vars.GIPNY_DHT_SEEDS`; libcore читает ту же переменную
   ещё и при запуске; джоба `e2e-dht` в `e2e-i2pd.yml` поднимает свой сид и
   гоняет `SessionManager` по сценарию «B ушёл → A пишет → A ушёл → B вернулся
   на новом адресе, прочитал, ответил → B ушёл → A вернулся, прочитал».
