...
2. **Локальная поверхность роутера и сеть релеев** (найдено 2026-09-24,
   записано в issues):
   - #101 — SAM без локальных портов: сначала Unix-сокет (патч i2pd по образцу
     `I2PControl`, vendored yosemite), затем i2pd библиотекой. **Отложено
     владельцем.** Решено: Windows остаётся на TCP с секретными ID сессий;
     локальный `make` i2pd разрешён только для проверки, что патч компилируется.
     Где держать патч i2pd — не решено.
