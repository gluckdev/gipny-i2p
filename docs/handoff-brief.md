...
1. **Фаза 5 сети релеев — код в ветке `feat/dht-seed`, ждёт CI и владельца.**
   ...
   **Нужно от владельца:** `age-keygen` → приватный ключ целиком в секрет
   ...
   headless-путь.
2. **0.4.4+ на настоящих устройствах.** Сеть релеев и новый интерфейс проверены
   на машине владельца и в превью; Android и macOS на железе никто не гонял.
3. **Голосовые сообщения** (инспекция, B2), установщики агента и архивы агента
   для macOS и Windows (план агента, §5–§6).
4. **Yggdrasil под капотом.** Просьба владельца (2026-09-17): тянуть его внутрь
...
5. **Изучить чужие открытые проекты и взять лучшее** (просьба владельца,
