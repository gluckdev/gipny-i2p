...
`e2e-dht` (см. ниже).
Прогон по живой i2p с последовательным офлайном обоих участников остаётся
частью фазы 5: без постоянного сид-узла этот сценарий нечем стабильно
bootstrap-ить.

ЧТО ЕЩЁ ОТКРЫТО, ПО ПОРЯДКУ

1. **Фаза 5 сети релеев — код в ветке `feat/dht-seed`, ждёт CI и владельца.**
   Где остановились 2026-09-24 и порядок до 0.4.11:
   `docs/plans/2026-09-24-dht-seed-status.md`.
   Сделано: `gipny-relay --dht` — узел сети, хранящий предметы и таблицу узлов
   в SQLite рядом с `relay.db` (`core/relay/src/dht.rs`, `dht_store.rs`);
   `relay-testnet.yml` запускает его сидом, а состояние переносит в кэше только
   зашифрованным `age` (новое имя кэша `dht-seed-state-*`: старые
   `relay-testnet-state-*` держали ключ открытым, поэтому у сида новая личность);
   `release.yml` вшивает `vars.GIPNY_DHT_SEEDS` (только при сборке: из
   окружения при запуске клиент сиды не берёт); джоба `e2e-dht` в `e2e-i2pd.yml` поднимает свой сид и
   передаёт его ботам через их таблицу узлов и гоняет `SessionManager` по
   сценарию «B ушёл → A пишет → A ушёл → B вернулся
   на новом адресе, прочитал, ответил → B ушёл → A вернулся, прочитал и узнал
   новый адрес B».
   **Нужно от владельца:** `age-keygen` → приватный ключ целиком в секрет
   `DHT_SEED_AGE_KEY` (публичный выводится из него); после первого запуска
   relay-testnet — его destination в переменную репозитория `GIPNY_DHT_SEEDS`.
   Не покрыто: тот же сценарий для приложения (`Core`) — e2e гоняет только
   headless-путь.
2. **Локальная поверхность роутера и сеть релеев** (найдено 2026-09-24,
   записано в issues):
...
6. **Изучить чужие открытые проекты и взять лучшее** (просьба владельца,
