...
1. **Сид сети релеев — ждёт владельца.** Фаза 5 влита (#98) и вышла в 0.4.11;
   ...
   **Нужно от владельца:** приватный ключ `age` целиком в секрет
   `DHT_SEED_AGE_KEY`; после первого запуска relay-testnet с ним — destination
...
2. **Скорость подключения** (ветка `perf/startup`, не влита, e2e не гонялся):
   свой встроенный релей читается внутри процесса по трубе
   (`EphemeralRelay::connect_local`, `relay::connect_local`, в `Core` и
   `SessionManager`), а не через i2p — минус 10–15 с до «подключено» и проход
   через сеть на каждое входящее письмо. Следующий шаг — строить туннели релея,
   пока вводится пароль (сейчас они строятся только после разблокировки, 36–39 с).
   Там же `docs/plans/2026-09-24-netdb-rendezvous-evaluation.md` (NetDb как L0).
3. **Локальная поверхность роутера и сеть релеев** (найдено 2026-09-24,
   записано в issues):
   - #101 — роутер без локальных портов. **Решение владельца: никаких портов
     вообще** — i2pd библиотекой через `libi2pd/api.h`, без SAM и HTTP-прокси.
     Unix-сокет отменён. План: `docs/plans/2026-09-24-embedded-i2pd.md`. Локальный
     `make`/компиляция i2pd разрешены только для проверки, что код компилируется.
   - #102 — HTTP-прокси обновлений открыт всем локальным программам.
...
- На Android поднят HTTP-прокси i2pd (`GipnyService.kt`, порт 4444, тот же
  ...
  доверять. Установку APK делает системный установщик: на Android найденная
...
- **Осталось, после 0.4.5** (подробно — в
  `docs/plans/2026-09-17-borrowing-from-others.md`, раздел про обновления): свой
  HTTP-клиент поверх SAM вместо локального прокси (на одном устройстве loopback
  общий, и пароль на прокси i2pd поставить нельзя) и дельта-обновления (zsync
  для AppImage), которые опираются на первый пункт.
