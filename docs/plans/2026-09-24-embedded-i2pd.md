...
- **Скрестившиеся X3DH-инициализации.** Без 10-секундного тайбрейкера обе стороны открывают сессию сразу, и инициализации могут скреститься. Остаётся сессия стороны с меньшим ключом подписи (`ours_stands`, одинаково в `core.rs` и `session.rs`). Сначала победитель выбрасывал вторую сессию вместе с письмами по ней — они ждали переотправки; теперь проигравшая сессия хранится, и письма по ней читаются. А контакт, не ответивший на нашу инициализацию, мог оказаться заперт: его новая инициализация (переустановка, сброс) «скрещивалась» с нашей устаревшей и проигрывала каждый раз. Вторая инициализация, пока первая отложена как проигравшая, — новое начало, она принимается (`4c8027e`, `cfa3f3f`; e2e с `E2E_BOTH_FIRST=1` в `i2p-embed.yml`). Проигравший больше не переотправляет всё сразу: победитель читает письма по его сессии, а разовая переотправка давала пять дублей из пяти (run 36038572639, `648234f`).
- **Повторный дозвон без письма.** Заранее релей собеседника дозванивался один раз; неудача (LeaseSet ещё не разошёлся) ждала первого письма, и письмо ждало свой дозвон — 7 с из 16 с эха (run 36042483601). Теперь после неудачи дозвон повторяется сам по бэкоффу, до пяти раз (`peer_relay_redial_after`). Там же: пока адрес своего релея не задан, пауза переподключения росла вхолостую, и первая настоящая попытка и её повтор шли с опозданием до 15 с.
- **Бандл заранее.** Первое письмо новому собеседнику ждало круг `GetBundle` (~3 с). Теперь бандл запрашивается по соединению с его релеем, как только оно открыто и сессии ещё нет, и хранится 2 мин: одноразовый prekey в нём могут успеть потратить (`6aed255`).
