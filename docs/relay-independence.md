...
Проверено по живой i2p в фазе 5 (ниже).

### Состояние (2026-09-24): фаза 5 влита, 0.4.11

- **Сид.** `gipny-relay --dht` — постоянный узел сети (`core/relay/src/dht.rs`,
  SQLite). Работает на воркере GitHub (`relay-testnet.yml`); состояние между
  запусками переносится только зашифрованным `age` (секрет `DHT_SEED_AGE_KEY`).
  Адрес вшивается в клиенты из переменной `GIPNY_DHT_SEEDS`, только при сборке.
  **Пока секрет и переменная не заданы, в релизах сида нет**, и две установки,
  перезапустившиеся обе, могут не найти друг друга.
- **Живой e2e** `e2e (relay network, each side away in turn)`: B уходит, A пишет
  и уходит, B возвращается на новом адресе, читает и отвечает, уходит, A
  возвращается, читает и находит новый адрес B. Зелёный на 0.4.11 (run
  35955326759).
- **Что этот e2e нашёл и что починено в 0.4.11** (в приложении было то же с 0.4.4):
  встроенный релей ломался от каждого закрытого соединения сети (accept в
  yosemite не cancel-safe); молчащий узел стоил 120 с на каждое письмо; запуск без
  связи стирал сохранённую таблицу узлов; обычные письма разбирались раньше
  вступительного и удалялись навсегда; письма обгоняли друг друга, пока шло
  подключение к релею собеседника.
- **Открыто:** узлы раздают непроверенных соседей (#99); идентификатор узла ничего
  не стоит — атака Сивиллы (#100).
- **Дальше:** поиск текущего адреса контакта через NetDb самой i2p (L0), а сеть
  релеев — только хранение писем (L1):
  `plans/2026-09-24-netdb-rendezvous-evaluation.md`. Решение за владельцем: ключ
  встречи на пользователя (присутствие видно всем, у кого карточка) или на пару.
