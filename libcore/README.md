...
- **Части — только через релей получателя**, не через сеть релеев (там 64 записи / 16 МБ на ключ). Повтор частей — только по дыре в `file_ack` или, если получатель подавал признаки жизни, по таймеру раз в 30 мин; иначе офлайн-получатель стоил бы гигабайт дублей на релее. Не больше `WINDOW` частей в полёте, считая повторы.
