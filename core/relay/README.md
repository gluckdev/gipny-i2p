...
- **Несколько соединений одного ключа.** Владелец может войти сразу несколькими (приложение так забирает части файла). Новая почта раздаётся по ним по кругу; догоняет и обходит раз в 30 с только первое. Каждое письмо отдаёт тот, кто первым пометил его в `Pushed` — приём (сразу после записи, до ответа отправителю, который ждёт i2p), обход после ack или догон; иначе обход, пришедший, пока отправителю ещё отвечали, отдавал письмо второй раз. Неподтверждённое закрытым соединением раздаётся оставшимся (`redeal`), а когда закрыто последнее — `Pushed` с нуля.
- **Кадр клиента читается вне `select!`.** Чтение (`read_frame`) живёт между оборотами цикла: оборванное на середине посреди отправки письма оно брало бы середину кадра за длину.
