...
- **Несколько соединений одного ключа.** Владелец может войти сразу несколькими (приложение так забирает части файла). Новая почта раздаётся по ним по кругу и считается отданной в момент постановки в очередь; догоняет и обходит раз в 30 с только первое. Неподтверждённое закрытым соединением раздаётся оставшимся (`redeal`), а когда закрыто последнее — курсор с нуля.
