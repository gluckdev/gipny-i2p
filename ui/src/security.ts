    ...
    this.el = h('div', { class: 'modal', style: { maxWidth: '760px' } },
      ...
      h('div', { class: 'modal-body' },
        ...
        h('div', {
          ...
          'Идентификатор юзера — только два публичных ключа и i2p-адрес. Ни телефона, ни email, ни IP, ни username — регистрация невозможна в принципе. ',
        ...
        h('div', {
          ...
          h('div', { style: { fontFamily: 'monospace', color: '#5cff5c', marginBottom: '8px' } },
            '[ твоё устройство ]',
            h('br', null), '   ↓ (опционально) системный VPN',
            h('br', null), '   ↓ loopback → встроенный роутер i2pd',
            h('br', null), '   ↓ исходящие туннели i2p, чесночная маршрутизация',
            h('br', null), '[ gipny-relay, i2p destination ] — НЕ видит контент, НЕ видит отправителя',
            h('br', null), '   ↓ (через тот же релей, обратно по i2p)',
            h('br', null), '[ destination получателя ]',
            h('br', null), '   ↓ Double Ratchet decrypt',
            h('br', null), '[ его устройство ]',
          ),
          h('div', { class: 'hint', style: { fontSize: '11px' } },
            'Провайдер видит, что ты подключён к узлам i2p (но не куда и не что). Системный VPN добавляет ещё один слой между твоим IP и входом в i2p. ',
            'Внешний SOCKS5-прокси убран: до локального роутера трафик идёт по loopback, а маршрутизацией между узлами занимается сам роутер. ',
        ...
        featureRow('Transport: встроенный роутер i2pd',
          'Роутер поставляется внутри приложения и поднимается на loopback (SAMv3) — ставить i2p отдельно не нужно. Консоль, прокси и UPnP выключены. Provider видит подключение к узлам i2p, но не куда ты ходишь.'),
        featureRow('Эфемерный адрес',
          'i2p-destination генерируется заново на каждый запуск и на диск не пишется. Стабильная личность — ключи в хранилище: релей маршрутизирует по ним, поэтому сессии не связываются между собой на сетевом уровне.'),
        ...
        featureRow('Watchdog reconnect (75s)',
          'Если коннект к relay умер, через 75 секунд автоматический реконнект. Параметр выверен под мобильную сеть — на проводе быстрее, но на 4G RTT иногда >30s.'),
        ...
        featureRow('Подписанные обновления (Ed25519)',
          'Каждый релиз подписан offline-ключом, и клиент отвергает любой бинарь без валидной подписи. Сервер обновлений тоже внутри i2p — нет clearnet-следа при скачивании патчей. ВНИМАНИЕ: сервер обновлений пока не развёрнут, автообновление не работает.'),
        ...
        h('div', {
          ...
          h('div', { style: { marginBottom: '6px' } }, '◇ i2p, как и любая сеть с низкой задержкой, уязвим к timing-correlation атакам при глобальном наблюдателе (NSA-уровень). Системный VPN перед входом в сеть сужает поверхность атаки, но не устраняет её.'),
        ...
        h('div', { class: 'card-block', style: { fontSize: '11px', lineHeight: '1.9' } },
          ...
          h('div', null, 'transport:          i2p (встроенный роутер i2pd, SAMv3)'),
          h('div', null, 'optional outer:    системный VPN перед входом в i2p'),
