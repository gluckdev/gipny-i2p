...
Цена такого выбора — **доставка идёт, пока открыты оба приложения**: встроенный релей живёт вместе с приложением, почтового ящика «на потом» у него нет. Письмо контакту, который сейчас не в сети, остаётся в очереди отправителя и уходит, когда контакт появится и сообщит свой новый адрес. Если вы оба перезапустили приложения, ни разу не оказавшись в сети одновременно, адреса друг друга устарели у обоих — в чате появится пометка «контакт недоступен»; обменяйтесь свежими карточками (*моя карточка → Copy card*, у собеседника — добавить контакт заново: история и ключи сохраняются, обновляется только адрес).

Нужен ящик, который принимает почту, пока вас нет, — переключите *Настройки → relay* на **«внешний»** и укажите адрес выделенного релея (см. «Запуск собственного выделенного релея»). У `gipny-agent` то же самое: своя личная релей по умолчанию, `--relay` — внешний, для тех, кто хочет ящик «на потом».
...
4. **Встроенный релей — только онлайн-доставка:** адрес релея меняется при каждом запуске, и почта принимается, только пока приложение открыто. Для офлайн-ящика нужен внешний релей; на Android система может усыпить приложение вместе с релеем, поэтому для телефона внешний релей надёжнее.
