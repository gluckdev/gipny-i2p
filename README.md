...
> ⚠️ **Статус на 2026‑07‑19: транспорт не доставляет сообщения.** go‑i2p не
> достраивает клиентские туннели, поэтому анонимного транспорта сейчас нет —
> ни в CI, ни на живой машине. Всё описанное ниже про эфемерные адреса и
> туннели — это то, как задумано, а не то, что работает сегодня. Замеры,
> причины и решение по замене роутера на i2pd:
...
- **Отладочный для теста** — артефакты джобы `android APK` workflow **build**:
  собирается на каждый push в main и каждый PR (Actions → build → последний
  запуск → Artifacts). Два варианта: `gipny-android-debug-apk` (`arm64-v8a`,
  реальные устройства) и `gipny-android-debug-apk-x86_64` (эмулятор). Джоба
  **android emulator smoke** прогоняет x86_64‑сборку на холодном эмуляторе
  (API 33) и проверяет старт `GipnyService` + SAM‑мост; помечена
  `continue-on-error`, так как флаки эмулятора не должны красить основную сборку.

Обновляется приложение на Android через собственный update‑канал (артефакты
`android-apk-<arch>` в манифесте сервера обновлений), Google Play не используется.
...
- **`.github/workflows/build.yml`** — проверка на каждый push/PR: матрица go‑роутера (linux amd64/arm64, windows) + Rust‑workspace + typecheck UI + джоба **android APK** — собирает debug‑APK для `arm64-v8a` и `x86_64` (эмулятор), проверяет, что `libgipnyi2p.so` попал внутрь каждой сборки, и выкладывает их отдельными артефактами; следом необязательная (`continue-on-error`) джоба **android emulator smoke** на x86_64‑сборке.
...
- **`.github/workflows/e2e.yml`** — сквозной тест «от А до Я»: поднимает релей и двух headless‑ботов (`bot-sdk`) поверх реальных i2p‑туннелей, шлёт сообщения и меряет latency/RSS в Job Summary. Тяжёлый (реальные туннели, 15–30 мин), поэтому крутится по расписанию (ночью) и вручную (`workflow_dispatch`), а не на каждый push/PR. Релей и оба бота работают на **одном общем роутере** (`GIPNY_SAM_PORT`): `go-i2cp` прибит к `127.0.0.1:7654`, поэтому рабочий I2CP‑транспорт может быть только у одного роутера на хост. Пока `continue-on-error` — сеть остаётся ненадёжной (reseed, построение туннелей), и зелёная галка сама по себе ничего не значит: смотреть надо на строку `PASS`/`FAIL` в step summary и на `echoes received` в таблице `[e2e-timing]`.
