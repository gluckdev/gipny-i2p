...
> Это I2P-форк оригинального мессенджера gipny. Сетевой транспорт полностью переведён с Tor (Arti) на **I2P** (на базе C++ роутера `i2pd`, вкомпилированного в каждый бинарь как библиотека). Криптографическое ядро, схема Double Ratchet, зашифрованный вольт SQLCipher и интерфейс сохранены.
...
```
          ...
          ├──────────────────────────┼───────────────────────────────┤
          │ 🧅 Сеть и Анонимность    │ i2pd в процессе, без портов   │
...
- 🌐 **Встроенный I2P-роутер (`i2pd`):** Не нужно настраивать сторонние демоны или прокси. `libi2pd` вкомпилирован в приложение, агент и релей (крейт `i2p-embed`) и работает в их же процессе: ни дочернего процесса, ни SAM, ни одного слушающего локального порта.
- 🎭 **Эфемерный сетевой след:** Адрес узла генерируется заново при каждом запуске (новые ключи destination на встроенном роутере) и хранится только в памяти. На диск сетевой адрес не пишется. Исходящая сессия клиента не публикуется (`publish: false`); публикуется только адрес встроенного релея — он тоже новый при каждом запуске.
...
- Вложения: файлы и картинки, галерея «медиа и файлы» по чату, вставка
  картинки из буфера обмена, сохранение на диск (0.4.0). Файлы больше 128 КБ
  идут частями по 192 КБ и собираются у получателя в любом порядке: до 512 МБ
  из приложения, с полосой прогресса и отменой, передача переживает обрыв и
  перезапуск; части идут сразу по нескольким потокам до релея получателя
  (следующая версия). Раньше вложение ехало одним письмом, и больше ~4 МБ не
  доходило.
...
- Отправка сразу, не дожидаясь своего релея (0.4.6).
- Первые письма тому, кого нет в сети, доходят вслед за знакомством; письма не
  обгоняют друг друга; недоступный узел сети не тормозит каждое письмо;
  запомненные узлы сети не стираются запуском без связи (0.4.11).
- Агент и боты доставляют через сеть релеев, как приложение; групповые
  рассылки ботов стоят в очереди до отправки (0.4.11).
- Встроенный релей держится при любом числе коротких соединений сети релеев
  (0.4.11).
...
- Перезапуск умершего роутера сам, без перезапуска приложения (0.4.6; с роутером
  внутри процесса неактуально: падение роутера — падение приложения).
- Обновления нельзя подменить чужим корневым сертификатом (0.4.7).
- Туннели строятся, пока вы набираете пароль (0.4.10).
- Секретные имена сессий у роутера: чужая программа на том же устройстве не
  может подключиться к нашей сессии и перехватить соединения (0.4.11; SAM с тех
  пор убран совсем, см. ниже).
- Роутер внутри процесса: ни SAM, ни HTTP-прокси, ни одного слушающего порта;
  обновления идут своим потоком i2p к outproxy; сертификаты reseed вшиты и
  проверяются (следующая версия).
- `gipny-relay --dht`: выделенный релей как постоянный узел сети релеев
  (0.4.11).
...
- Android: «назад» убирает приложение с экрана, а не завершает его; падения
  при сворачивании и на проверке обновлений устранены (0.4.7, 0.4.8).
- Строка состояния канала над чатом — измеренная, а не угаданная; «кокаин»
  внутри «кофеина»: путь на один узел короче, с честно названной ценой (0.4.9).
- «В сети» — только по живому сигналу; релей говорит, что с ним не так (0.4.10).
...
|---|---|---|
| [0.4.13](https://github.com/gluckdev/gipny-i2p/releases/tag/v0.4.13) | Письмо в сети, только когда его взял другой узел; единственный узел не выпадает после неудачи; свой релей не зависает | [0.4.13.md](docs/releases/0.4.13.md) |
| [0.4.12](https://github.com/gluckdev/gipny-i2p/releases/tag/v0.4.12) | Своя почта читается напрямую, без i2p; телефон не теряет сеть после смены Wi-Fi/LTE; первое письмо после запуска уходит быстрее | [0.4.12.md](docs/releases/0.4.12.md) |
| [0.4.11](https://github.com/gluckdev/gipny-i2p/releases/tag/v0.4.11) | Встроенный релей больше не глохнет; первые письма тем, кого нет в сети, доходят; агент и боты пишут через сеть релеев | [0.4.11.md](docs/releases/0.4.11.md) |
...
2. **Роутер внутри процесса (`i2p-embed`):** `libi2pd` вкомпилирован в каждый бинарь; C-прослойка над `libi2pd/api.h` (`i2p-embed/shim`) и обёртка на Rust (`Router`, `Destination`, `I2pStream`). Один роутер на процесс (`libcore/src/embedded.rs`, данные в `<профиль>/i2p/router`). Ни SAM, ни HTTP-прокси, ни одного слушающего локального порта — посторонней программе на устройстве не к чему подключиться ([план](docs/plans/2026-09-24-embedded-i2pd.md), #101, #102). Автообновление идёт своим потоком i2p к outproxy, TLS внутри (`libcore/src/i2p_http.rs`).
3. **Чесночные туннели:** `i2pd` строит исходящие туннели через распределенную сеть узлов I2P.
4. **Android:** `libi2pd` и та же прослойка собираются в `libi2pd.so` (`android-router/jni`), Rust-часть приложения линкуется с ней и запускает роутер тем же путём, что десктоп. `GipnyService.kt` только держит foreground-сервис и сообщает роутеру о смене сети.
...
**Встроенный релей (по умолчанию).** Настраивать ничего не нужно: каждое приложение при запуске поднимает релей внутри себя и получает почту через него — напрямую, внутри процесса, а не через i2p (собеседники, конечно, приходят к нему через i2p). Адрес этого релея создаётся заново при каждом запуске и нигде не сохраняется — постоянного адреса, по появлениям которого в сети можно было бы вычислить часы вашей активности, не существует. Релей личный: он хранит письма и prekey-пакет только своего владельца и отказывает всем остальным. После запуска релею нужно 1–2 минуты на постройку туннелей; затем приложение само сообщает новый адрес всем, с кем уже есть переписка.

**Когда релей получателя молчит — письмо идёт через сеть релеев.** Встроенный релей живёт вместе с приложением, поэтому адресата может не быть на месте. В этом случае клиент запечатывает письмо и оставляет его в сети релеев (`dht/`, `libcore/src/dht_client.rs`): узлы хранят шифротекст до семи дней под ключом, который никто, кроме этих двух собеседников, не вычислит. Адресат забирает письмо при следующем запуске, подтверждает получение и удаляет его из сети. Оттуда же берутся его текущий адрес (если он перезапускался) и prekey-пакет для самого первого сообщения. Живой тест `e2e-dht` проверяет это целиком: B уходит, A пишет и уходит, B возвращается на новом адресе, читает и отвечает, уходит, A возвращается на новом адресе, читает ответы и узнаёт новый адрес B.

**Как приложение находит сеть релеев.** Через релеи контактов, запомненные с прошлых запусков узлы и вшитые в сборку сиды (`GIPNY_DHT_SEEDS`, переменная репозитория; только при сборке). Сидом служит `gipny-relay --dht` на воркере GitHub Actions (`relay-testnet.yml`), состояние которого переносится между запусками только зашифрованным `age`. Пока адрес сида не вшит в релиз, две установки, перезапустившиеся обе, могут не найти друг друга. Дальше — поиск адреса контакта через NetDb самой i2p ([разбор](docs/plans/2026-09-24-netdb-rendezvous-evaluation.md)).
...
|---|---|---|
| `libcore/` | Общее ядро: криптография, сессии Double Ratchet, база, протокол релея, встроенный релей, запуск роутера внутри процесса, автообновление | [libcore/README.md](libcore/README.md) |
...
| `i2p-embed/` | Роутер i2pd внутри процесса: C-прослойка над `libi2pd/api.h`, обёртка на Rust, сборка libi2pd (`build.rs`) | — |
| `android-router/` | `libi2pd.so` для Android: libi2pd + прослойка + одна точка входа JNI | [android-router/README.md](android-router/README.md) |
...
| `tools/` | Иконки, конвертеры конфигов sing-box | [tools/README.md](tools/README.md) |
...
- **Node.js:** 18+ и `npm`
- **C++ компилятор, Boost (program_options), OpenSSL, zlib** — `libi2pd` компилируется прямо в бинарь (`i2p-embed/build.rs`, переменные `I2P_EMBED_*`)
...
# Глоб ресурсов в tauri.conf.json (resources/i2pd*) должен что-то найти.
# В релизе там снимок netDb (i2pd-netdb-seed.tar.gz); для dev хватит заглушки.
mkdir -p core/resources && touch core/resources/i2pd-placeholder

# Сборка фронтенда
cd ui && npm install && npm run build && cd ..

# Запуск в dev-режиме: роутер вкомпилирован, отдельный i2pd не нужен
cd core
cargo run
```
...
```bash
# Сборка релей-сервера (отдельный workspace)
cargo build --release --manifest-path core/relay/Cargo.toml

# Запуск: роутер внутри процесса, его данные в $GIPNY_RELAY_DATA/router,
# ключи адреса — в dest.key; отдельный i2pd не нужен; SIGTERM/Ctrl-C — чистая остановка
GIPNY_RELAY_DATA=/var/lib/gipny-relay ./core/relay/target/release/gipny-relay

# То же, и узел сети релеев: хранит запечатанные письма для других,
# переживает перезапуск; --seeds / GIPNY_DHT_SEEDS — через кого войти в сеть
GIPNY_RELAY_DATA=/var/lib/gipny-relay ./core/relay/target/release/gipny-relay --dht
```
...
Результаты сквозного тестирования в живой сети I2P, версия 0.4.11 ([e2e-i2pd run 35955326759](https://github.com/gluckdev/gipny-i2p/actions/runs/35955326759); все боты на одном общем роутере CI, уже прогретом):

| Параметр | Значение | Примечание |
| :--- | :--- | :--- |
| **Сценарии** | **5 из 5** | два бота через выделенные релеи; через встроенные релеи; бинарь агента; сеть релеев, стороны уходят по очереди |
| **Адрес клиента готов** | **12–27 с** | после запуска роутера (замер 0.4.11, тогда ещё SAM-сессия) |
| **Встроенный релей поднят** | **36–39 с** | туннели + публикация адреса |
| **Письмо туда и обратно** | **~42 с (медиана)** | через два встроенных релея, на 0.4.11; после него свой релей читается внутри процесса — минус один проход через i2p |
| **Оба по очереди офлайн** | **952 с сценарий целиком** | 3 письма и 3 ответа через сеть релеев, смена адресов, адрес находится |
| **Утечка IP-адреса** | **исключена** | приложение не открывает прямых сокетов в интернет |
| **Холодный старт** | **1–3 мин** | первый запуск: reseed и построение туннелей |
...
| **Устройство** | Роутер внутри процесса: ни SAM, ни HTTP-прокси, ни одного слушающего локального порта; сертификаты reseed вшиты и проверяются (`--reseed.verify=true`). | Код роутера (C++) работает в том же процессе: его падение роняет приложение. Транспорты самого роутера (NTCP2/SSU2) слушают, как у любого узла i2p. |
...
6. **Узлы сети релеев пока ничего не стоят:** можно завести много своих и окружить нужный ключ сети — прочитать или подделать письма так нельзя, но можно их прятать (#99, #100).
7. **Автообновление ходит на GitHub через внешний i2p-outproxy** (`exit.stormycloud.i2p`), а не напрямую: приложение и агент не светят свой настоящий IP перед GitHub, но полагаются на сторонний узел, который видит, что кто-то сейчас обращается к api.github.com (содержимое — нет, там TLS). Проверка идёт в фоне сама, без вопросов; результат ставится сразу, но запускается только со следующего запуска. Отключить — *Настройки → обновляться автоматически*. У api.github.com лимит 60 запросов в час на IP без токена, и этот IP у outproxy общий на всех, кто через него сейчас ходит — в час пик проверка может молча не пройти и повториться позже, это не авария.
...
- Роутер i2p вкомпилирован и работает в его же процессе (без SAM и портов); агент получает эфемерный адрес и ждёт команды мастера. SIGTERM (остановка службы) — как Ctrl-C: мастеру уходит `REVOKE`, роутер останавливается до выхода.
...
Архив `gipny-agent_<версия>_linux-<arch>.tar.gz` со страницы релиза содержит `gipny-agent` (роутер внутри), `i2pd-netdb-seed.tar.gz` (снимок известных узлов i2p для быстрого первого запуска — держать рядом с бинарём), юнит `gipny-agent.service` и `README.txt` с шагами установки.
