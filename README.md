...
- 🎭 **Эфемерный сетевой след:** Адрес узла генерируется заново при каждом запуске сессии (`DEST GENERATE`) и хранится только в `Zeroizing`-памяти. На диск сетевой адрес не пишется. Исходящая сессия клиента не публикуется (`publish: false`); публикуется только адрес встроенного релея — он тоже новый при каждом запуске.
...
- Перезапуск умершего роутера сам, без перезапуска приложения (0.4.6).
...
- Секретные имена сессий у роутера: чужая программа на том же устройстве не
  может подключиться к нашей сессии и перехватить соединения (0.4.11).
...
2. **Локальный SAMv3-мост (`127.0.0.1`, свободный порт):** Ядро общается с `i2pd` через SAMv3 при помощи крейта `yosemite`. У SAM в i2pd нет аутентификации, поэтому имя каждой сессии содержит 128 случайных бит — иначе чужая программа на том же устройстве могла бы подключиться к нашей сессии. Цель — никаких локальных портов вообще: i2pd внутри процесса через его C++ API ([план](docs/plans/2026-09-24-embedded-i2pd.md), #101, #102).
...
# Запуск (подключится к запущенному SAMv3 на порту 7656; порт — GIPNY_SAM_PORT)
...
| **SAM-сессия клиента** | **12–27 с** | после запуска роутера |
...
| **Устройство** | Роутер слушает только `127.0.0.1`; имена SAM-сессий секретные (128 случайных бит). | Любая программа на устройстве (на Android — любое приложение) может завести свою сессию на нашем роутере и пользоваться HTTP-прокси обновлений. Уйдёт вместе с портами (#101, #102). |
