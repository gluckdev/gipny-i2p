...
- **`android emulator smoke` сейчас всегда красный, но ничего не блокирует** (`continue-on-error: true`). Эмулятор загружается нормально, падает установка: `adb install` получает пустой путь (в логе `Installing ` без файла). `reactivecircus/android-emulator-runner` выполняет каждую строку `script` отдельной командой `sh -c`, поэтому переменная `APK=$(find …)` из первой строки не доживает до `adb install`. Лечится записью шагов в одну строку или вызовом скрипта-файла.
